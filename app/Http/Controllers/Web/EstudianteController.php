...
class EstudianteController extends Controller
{
    public function jefeGenerateScheduleForStudent(Request $request): JsonResponse
    {
        ...
        if ($totalCredits > 29) {
            return response()->json(['message' => "El estudiante excede el límite de 29 créditos (Total: $totalCredits)."], 422);
        }
            ...
            foreach ($inscripciones as $i) {
                ...
                if ($asignacion && $asignacion->id_bloque) {
                    ...
                    if (isset($bloquesOcupados[$key])) {
                        throw new \Exception("Colisión: '{$i->materia->nombre}' y '{$bloquesOcupados[$key]}' coinciden en el mismo bloque.");
                    }
